...
const setHoursToDate = (initDate: string, hours: string) => {
...
const EventModal = ({ updateCalendar }: EventModalProps): React.ReactElement => {
    ...
    const isNewEvent = useAppSelector(({ eventModalController }) => eventModalController.new)
    ...
    }, [dispatch, eventInfo?.id, currUserId])
    ...
    }, [dispatch, eventInfo, updateCalendar, userInfo.id])
                ...
                <div className={'event-modal__body'}>
                    {isNewEvent ? <>
                        ...
                        <div className={'event-modal__info-block'}>
                            ...
                            <div>Last updated by: {eventInfo?.blocked_by}; </div>
                            <div>Last updated at: {new Date(eventInfo?.updated_at || 0).toLocaleString()}</div>
                        </div>
                    </>}
                </div>
                <div className={'event-modal__footer'}>
                    <button className={'event-modal__submit-button event-modal__form-button'} onClick={updateEventPress}>Submit</button>
                    <button className={'event-modal__delete-button event-modal__form-button'} onClick={deleteEventPress}>Delete</button>
                </div>
            </div>
        </div>}
    </>
